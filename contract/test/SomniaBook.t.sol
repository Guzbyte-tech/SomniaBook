...
contract SomniaBookTest is Test {
    ...
    function testPlaceBetTooLarge() public {
        ...
        vm.expectRevert("Bet amount too large");
        vm.prank(user1);
        betting.placeBet{value: betAmount}(MARKET_ID, SELECTION_CHELSEA, DEFAULT_ODDS);
    }
    ...
    function testPlaceBetEmptyMarketId() public {
        ...
        vm.expectRevert("Market ID required");
        vm.prank(user1);
    ...
    function testPlaceBetEmptySelection() public {
        ...
        vm.expectRevert("Selection required");
        vm.prank(user1);
    ...
    function testResolveBetsAsWin() public {
        ...
        vm.prank(owner);
        betting.resolveBetsAsWin(winningBets);
    ...
    function testResolveBetsAsLoss() public {
        ...
        vm.prank(owner);
        betting.resolveBetsAsLoss(losingBets);
    ...
    function testClaimWinningsNotResolved() public {
        ...
        vm.expectRevert("Bet not resolved yet");
        vm.prank(user1);
        betting.claimWinnings(betId);
    }
    ...
    function testClaimWinningsLosingBet() public {
        ...
        bets[0] = betId;
        vm.prank(owner);
        betting.resolveBetsAsLoss(bets);
    ...
    function testLargeBatchResolution() public {
        ...
        // Resolve all as wins
        vm.prank(owner);
        betting.resolveBetsAsWin(betIds);
    ...
    function testPrecisionWithSmallAmounts() public {
        uint256 smallBet = betting.minBetAmount(); // 0.001 ether
        uint256 odds = 1001; // 1.001x odds
        
        vm.prank(user1);
        bytes32 betId = betting.placeBet{value: smallBet}(MARKET_ID, SELECTION_CHELSEA, odds);
        
        (uint256 grossPayout, uint256 fee, uint256 netPayout) = betting.calculatePayout(betId);
        
        // Even with small amounts, math should work correctly
        assertTrue(grossPayout >= smallBet);
        assertTrue(fee <= grossPayout);
        assertTrue(netPayout <= grossPayout);
        assertEq(netPayout + fee, grossPayout);
    }

    function testPrecisionWithLargeAmounts() public {
        uint256 largeBet = betting.maxBetAmount(); // 10 ether
        uint256 highOdds = 9999; // 9.999x odds
        
        vm.deal(user1, largeBet);
        vm.prank(user1);
        bytes32 betId = betting.placeBet{value: largeBet}(MARKET_ID, SELECTION_CHELSEA, highOdds);
        
        (uint256 grossPayout, uint256 fee, uint256 netPayout) = betting.calculatePayout(betId);
        
        // Verify calculations are correct for large amounts
        uint256 expectedGross = (largeBet * highOdds) / 1000;
        uint256 expectedFee = (expectedGross * 250) / 10000;
        uint256 expectedNet = expectedGross - expectedFee;
